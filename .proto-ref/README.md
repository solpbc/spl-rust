...
To re-vendor, check out the intended upstream commit in a separate clean worktree and copy exactly `framing.md`, `pairing.md`, `pair-window.md`, `session.md`, and `tokens.md` from its `proto/` directory. Update the commit pin here, `PROTOCOL_REVISION` in `crates/spl-core/test-support/corpus_generator.rs`, and the per-document SHA-256 pins in `crates/spl-core/tests/source_policy.rs`. Then run `cargo run -p spl-core --example generate_conformance` followed by the repository gates. Do not copy upstream's `proto/README.md`, and do not edit the mirrored documents locally.

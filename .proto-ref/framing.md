...
The framing layer is **application-layer agnostic**. A `stream_id` is a labelled byte channel; it carries whatever bytes the endpoints agreed to send — HTTP/1.1 today, WebSocket frames after an upgrade, SSE events inside a long-lived HTTP response, or anything else. The framing code does not parse the payload; the home hands stream bytes to its in-process HTTP server, and the client bridges them from a local loopback port. That is agnosticism, not a privacy property. Both endpoints terminate the inner TLS and hold the plaintext by design — they have to, since they are the ones talking. The blind-by-construction claim is about the **relay**, which cannot read any of this because it would have to decrypt the **inner** TLS session, which it holds no key for; see *relationship to the relay*.

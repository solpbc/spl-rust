...
**Addressing — the DO identity is the rendezvous match.** Both endpoints carry `RK` **in the upgrade request** (a request header, e.g. `Sec-Pair-Key: <RK hex>` — *not* the URL query, to keep `RK` out of edge access logs; routing happens at the HTTP layer, before any WS frame). Header-less clients that cannot set `Sec-Pair-Key` MAY carry `RK` on `/session/pair-dial` as `Sec-WebSocket-Protocol: spl-v1, spl-pair.<32-hex>`; the relay selects exactly `spl-v1` on the 101 and never echoes the `spl-pair.<RK>` token. `RK` MUST NOT be accepted via `?rk=` query fallback (the existing `?token=` fallback for service/device tokens must not be extended to `RK`). The Worker routes by `idFromName(RK_hex)` → a **distinct DO instance** addressed by `RK`. **Reuse the existing per-instance DO class** (it already carries the bridge machinery) addressed by `idFromName(RK)` rather than introducing a new DO class/binding; the RK-addressed instance is the ephemeral "pairing DO". Home and client presenting the same `RK` land in the **same DO instance** — that *is* the match. The relay performs **no `RK` value comparison** and stores no `RK` secret; it routes and bridges whoever is in the DO. For irreversible instance retirement, the relay MAY persist the opaque 64-hex Durable Object ID produced by `idFromName(RK)` alongside the authenticated `instance_id` for the lifetime of sockets in that object; it never persists `RK` itself, and the stored ID is not accepted as `RK` by the public router. The pairing endpoints (`/session/pair-window`, `/session/pair-dial`, and the pairing `/tunnel/<id>`) carry **no `?instance=`** — the Worker's instance-param guard must not apply to them. `RK` is never logged (log `instance_id` from the home's token + reason only), whether carried in `Sec-Pair-Key` or the pair-dial subprotocol carrier.
...
The client derives `RK = HKDF(S, …)` and opens this WebSocket carrying `RK` in the upgrade request. Native clients use `Sec-Pair-Key: <RK hex>`. Header-less browser clients use `Sec-WebSocket-Protocol: spl-v1, spl-pair.<32-hex>` and receive a 101 response selecting exactly `spl-v1`. No JWT.
...
- If both `Sec-Pair-Key` and `spl-pair.<RK>` are offered and they disagree, or if `spl-pair.<RK>` is offered without `spl-v1`, reject with the same coarse `401`.

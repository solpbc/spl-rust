...
The catch-all is deliberately one rule rather than a table, because it has to be right without knowing which home it is talking to. `internal_error` (80) lands here, so a device unpaired by a home predating this section is still told, eventually. A transient chain error the home reports also lands here and resolves before it can persist. And a *persistent* problem the home reports, such as a corrupted keychain entry, lands here too, where a re-pair is the remedy anyway. ⛔ **Do not narrow this to a set of recognized codes.** A client that treats unrecognized codes as transient forever leaves an unpaired owner with no re-pair affordance, which is worse than not implementing the split at all.

A home that regenerated its CA does **not** land here either. It sends no refusal: when the device reaches it at a saved address, the client's own check of the home's certificate fails first, which is the case under *a peer that is not the paired home* below. Through the relay the device does not reach it at all, because its new CA gives it a new jid, and so a new `instance_id` ([`tokens.md`](tokens.md)); the device's dial finds no home, which reads as unreachable.
...
**Only an alert that arrives after the client has confirmed the home and sent its Finished is a verdict.** The client MUST check the home's certificate against the pinned CA ([`pairing.md`](pairing.md)) inside the handshake, before it sends its own Finished, not after the connection is up. In TLS 1.3 the home checks the client certificate after that Finished, so its refusal reaches the client on the first read that follows, under keys the confirmed home holds. An alert that arrives before both conditions hold is not authenticated: any peer on the path can forge one. A client MUST NOT act on it under this section; it is an ordinary handshake failure and is retried like one. Otherwise anyone who can answer at a device's saved address, or sit where the relay sits, could unpair that device with a single forged `access_denied`.

**A peer that is not the paired home.** When the certificate a peer presents does not chain to the pinned CA, the client ends the handshake itself. The home has said nothing, so none of the branches above apply: the client MUST NOT count this toward the last branch, and MUST NOT stop retrying because of it. It SHOULD tell the owner at once that something it could not confirm as their home answered, and where it answered: the direct address, or the relay. Until a handshake with the home completes on any path, that notice SHOULD also offer a re-pair while the client keeps retrying.

- When that peer presented a self-signed P-256 CA of its own, the client SHOULD also show that CA's identity ([`identity.md`](identity.md)), in the form the home shows its owner, next to the identity of the home it is paired with. The two differ in both cases, so the comparison that decides is the owner's: if the peer's identity matches what their home shows them now, the home regenerated its CA and the device needs a re-pair; if not, something else answered at that address.
- That identity is only what the peer presented. Any peer can present another journal's public certificate, so a client MUST NOT act on it beyond showing it.
- A certificate that does chain to the pinned CA but fails another check, such as its validity dates against a wrong clock, is not this case: the peer is the paired home, and the failure is an ordinary handshake failure.

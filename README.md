...
| **`spl-bridge`** | Public SNI-passthrough MCP relay for the journal-MCP endpoint, routing opaque client TLS bytes to a registered journal. Operational runbooks and deploy assets are in [`crates/spl-bridge/deploy/RUNBOOK.md`](crates/spl-bridge/deploy/RUNBOOK.md). |
